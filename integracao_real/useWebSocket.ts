...
export function useWebSocket({
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      wsRef.current.onclose = (event) => {
        ...
        // Reconexão automática se habilitada e não foi fechado intencionalmente
        if (enableAutoReconnect && shouldReconnectRef.current && event.code !== 1000 && reconnectAttemptsRef.current < maxReconnectAttempts) {
  ...
  }, [url, isConnecting, onConnect, onDisconnect, onError, joinRoom, enableAutoReconnect, maxReconnectAttempts, reconnectInterval]);
